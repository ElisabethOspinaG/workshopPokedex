...
function printInfoPokemonInTable(pokemonInfo) {
  ...
  // PINTAR EL TYPE
  const typePokemon = pokemonInfo.types[0].type.name;
  ...
  // PINTAR EL HIGHT
  const heightPokemon = pokemonInfo.height;
  ...
  // PINTAR EL WEIGGHT
  const weightPokemon = pokemonInfo.weight;
...
// llamando la funcion para llenar la tabla LIZ 
printInfoPokemonInTable (dataPokemon);
...
// ----------------------------------- APLICAR FUNCIONALIDAD / DIEGO ------------------------------
  ...
  nombrePokemonElemento.innerHTML = `${nombrePokemon.toUpperCase()}`;
    ...
    //------- Reemplazar la información de la tabla con la del Pokemon seleccionado con la funcion liz----------------
    
   printInfoPokemonInTable (pokemonDetail);
  });
